# Minor tweaks to the Oxide theme for Zsh by Pranav (pranav7)
#
...
if [[ "${terminfo[colors]}" -ge 256 ]]; then
    oxide_turquoise="%F{159}"
    oxide_orange="%F{178}"
    oxide_red="%F{161}"
    oxide_limegreen="%F{154}"
    oxide_yellow="%F{227}"
else
...
FMT_ACTION="(%{$oxide_yellow%}%a%{$oxide_reset_color%})"
FMT_VCS_STATUS="%{$oxide_red%} %b%u%c%{$oxide_reset_color%}%}"
...
# Oxide prompt style.
PROMPT=$'\n%B%{$oxide_yellow%}%~%{$oxide_reset_color%} ${vcs_info_msg_0_}%b\n%(?.%{%F{white}%}.%{$oxide_red%})%(!.#.❯)%{$oxide_reset_color%} '
